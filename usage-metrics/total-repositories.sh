...
while [ $COUNTER -lt $NUM_REPOS ]; do
	...
	echo "Repository $(cat repos.txt | jq -r ".[$COUNTER].name"); Storage: $STORAGE Downloaded: $DL in timeslice"
	echo "$(cat repos.txt | jq -r ".[$COUNTER].name"), $STORAGE, $DL, $(bc -l <<< $STORAGE/$TOTAL_STORAGE), $(bc -l <<< $DL/$TOTAL_DL)" >> usage.csv
    else
